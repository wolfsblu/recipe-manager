            ...
            <div class="grid lg:grid-cols-2 gap-6">
                    ...
                    <Form.Field {form} name="name">
                        <Form.Control>
                            {#snippet children({ props })}
                                <Form.Label>{m.recipes_form_recipeName()}</Form.Label>
                                <Input
                                    {...props}
                                    bind:value={$formData.name}
                                    placeholder={m.recipes_form_recipeNamePlaceholder()}
                                />
                            {/snippet}
                        </Form.Control>
                        <Form.FieldErrors />
                            ...
                            <Form.Control>
                                {#snippet children({ props })}
                                    <Form.Label class="flex items-center gap-2">
                                        ...
                                        {m.recipes_form_cookTime()}
                                    </Form.Label>
                                    <Input
                                        {...props}
                                        bind:value={$formData.minutes}
                                        type="number"
                                        placeholder={m.recipes_form_cookTimePlaceholder()}
                                        min="1"
                                    />
                                {/snippet}
                            </Form.Control>
                ...
                <div>
                    <Form.Field {form} name="description">
                        <Form.Control>
                            {#snippet children({ props })}
                                <Form.Label>{m.recipes_form_description()}</Form.Label>
                                <Textarea
                                    {...props}
                                    bind:value={$formData.description}
                                    placeholder={m.recipes_form_descriptionPlaceholder()}
        ...
        <Card class="border-0 md:border shadow-none md:shadow-sm">
            <CardHeader class="px-4 md:px-6">
                <CardTitle class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        {m.recipes_form_cookingInstructions()}
                    </div>
                    <Button type="button" onclick={addStep} variant="outline" size="sm">
                        <PlusIcon class="w-4 h-4" />
                        {m.recipes_form_addStep()}
                    </Button>
                </CardTitle>
            </CardHeader>
            <CardContent class="px-4 md:px-6">
                <div class="text-center py-12 text-muted-foreground">
                    ...
                    <p class="text-lg mb-2">{m.recipes_form_noStepsYet()}</p>
                    <p class="text-sm">{m.recipes_form_addFirstStep()}</p>
                </div>
            </CardContent>
        </Card>
    {:else}
                    ...
                    <CardHeader class="bg-muted/50 gap-0 py-1 px-4 md:px-6">
                        <div class="flex items-center justify-between">
                            <CardTitle class="flex items-center gap-3">
                                ...
                                {m.recipes_form_stepTitle({ number: stepIndex + 1 })}
                            </CardTitle>
                                            ...
                                            <div class="space-y-2 md:space-y-0 md:grid md:grid-cols-[2fr_2fr_3fr_auto] md:gap-2 md:items-start">
                                                        ...
                                                        <Form.Control>
                                                            {#snippet children({ props })}
                                                                <Input
                                                                    ...
                                                                    placeholder={m.recipes_form_amountPlaceholder()}
                                                                    class="text-sm"
                                                                />
                                                            {/snippet}
                                                        </Form.Control>
                                                ...
                                                <div class="md:flex md:justify-center md:items-start">
                                                    ...
                                                    >
                                                        ...
                                                        <span class="md:hidden">{m.recipes_form_delete()}</span>
                                                    </Button>
                                                </div>
                                            </div>
                                        {/each}
                                    ...
                                    <Form.Control>
                                        {#snippet children({ props })}
                                            <Textarea
                                                ...
                                                placeholder={m.recipes_form_instructionsPlaceholder()}
    ...
    <div class="flex justify-between items-center px-4 md:px-0">
        {#if onCancel}
            <Button type="button" variant="outline" onclick={onCancel}>
                {m.recipes_form_cancel()}
            </Button>
        {:else}
            <Button type="reset" variant="outline">
                {m.recipes_form_reset()}
            </Button>
        {/if}
